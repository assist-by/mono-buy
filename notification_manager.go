...
func generateDiscordEmbed(signalResult lib.SignalResult) *discord.Embed {
	koreaLocation, _ := time.LoadLocation("Asia/Seoul")
	timestamp := time.Unix(signalResult.Timestamp/1000, 0).In(koreaLocation)
	...
	case lib.SIGNAL_LONG:
		signalEmoji = "🚀 LONG"
		embedColor = discord.ColorGreen
	case lib.SIGNAL_SHORT:
		signalEmoji = "🔻 SHORT"
		embedColor = discord.ColorRed
	default:
		signalEmoji = "⏺️ NO SIGNAL"
		embedColor = discord.ColorBlue
	}
	...
	if signalResult.Signal == lib.SIGNAL_LONG || signalResult.Signal == lib.SIGNAL_SHORT {
		...
		description += fmt.Sprintf("**스탑로스**: $%.5f (%.5f%%)\n**목표가**: $%.5f (%.5f%%)\n",
			signalResult.StopLoss,
			stopLossPercent,
			signalResult.TakeProfit,
			takeProfitPercent)
	...
	embed := discord.NewEmbed().
		SetTitle(fmt.Sprintf("%s %s/USDT", signalEmoji, signalResult.Symbol)).
